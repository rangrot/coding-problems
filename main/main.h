#ifndef MAIN
#define MAIN

/**
 * @file main.h
 * @author rangrot
 * @brief This file contains the header for all the included problems
 * We have a convenient way to include just this single file in all the code
 *
 */
#include "stdint.h"
#include <stdio.h>
#include <string.h>
#include <math.h>
#include <limits.h>

#include "../arrayAndString/duplicatesInArray/duplicatesInArray.h"
#include "../arrayAndString/reverseOrderOfArray/reverseOrderOfArray.h"
#include "../arrayAndString/towSumProblem/twoSumProblem.h"
#include "../arrayAndString/sortedContinuesSubArray/sotredContinuesSubArray.h"
#include "../arrayAndString/dutchNationalFlag/dutchNationalFlag.h"
#include "../bitManipulationProblems/baseConversion/baseConversion.h"
#include "../bitManipulationProblems/flipBits/flipBits.h"
#include "../bitManipulationProblems/majorityNumber/majorityNum.h"
#include "../bitManipulationProblems/singleNumber/singleNumber.h"
#include "../linkedList/singlyLinkedList/singlyLinkedList.h"
#include "../linkedList/mergeTwoSortedList/mergeTwoSortedList.h"
#include "../sortingAlgorithms/sortingAlgorithms.h"
#include "../trees/buddyBitmap/buddyBitmap.h"


/**
 * @brief 
 * # defines for the project
 * for example the debug settings
 */
#define ENABLE_DEBUG 0

#endif
